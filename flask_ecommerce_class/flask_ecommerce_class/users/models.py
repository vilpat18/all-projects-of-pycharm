...
users_address = db.Table('user_addresses',
...
class User(db.Model):
    ...
    user_datails = db.relationship('UserDetails',backref='user',lazy='select',
                                   uselist=False,cascade="all,delete")
                                    # one-to-one relationship
    addresses = db.relationship('Address',secondary=users_address,backref='user',
                                lazy='joined' # use dynamic and check difference
                                  , cascade = 'all,delete' ) # many-to-many relationship

    def to_representation(self):
        if self.user_datails:
            user_detail = self.user_datails.to_representation()
        else:
            user_detail = {}
...
class Address(db.Model):
    ...
    id = db.Column('id',db.Integer,primary_key = True,autoincrement=True)
    city = db.Column('city',db.String(32),nullable=True)
